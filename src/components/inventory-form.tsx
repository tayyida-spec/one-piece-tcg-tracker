...
export function InventoryForm({ initial }: { initial?: InventoryFormValues }) {
  ...
  const [itemType, setItemType] = useState(initial?.itemType ?? "card");

  const [logTxnBuy, setLogTxnBuy] = useState(true);



  const isNewSealed = !initial?.id && itemType === "sealed";
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (isNewSealed && logTxnBuy) {

      body.logTxnBuy = true;

      body.purchaseDate = fd.get("purchaseDate");

      body.txnUnitPrice = fd.get("txnUnitPrice") || fd.get("purchasePrice") || null;

      body.displayId = fd.get("displayId") || null;
  ...
  return (
    ...
    <form onSubmit={onSubmit} className="max-w-xl space-y-4 rounded-lg border border-border bg-surface p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            defaultValue={initial?.itemType ?? "card"}
      ...
      {isNewSealed ? (
        ...
        <div className="space-y-3 rounded-md border border-border bg-surface-elevated/40 p-4">
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm font-medium">
            ...
            <input
              ...
              onChange={(e) => setLogTxnBuy(e.target.checked)}

              className="rounded border-border"

            />

            Log as transaction (shows on Transaction Log)
          ...
          {logTxnBuy ? (
            ...
            <>
              ...
              <p className="text-xs text-muted">
                ...
                Creates a Buy TXN and updates inventory. Uncheck for inventory-only (opening stock, corrections).
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="displayId" name="displayId" placeholder="TXN008 — auto if blank" />
                ...
                <div className="space-y-2">
                  ...
                  <DateInput id="purchaseDate" name="purchaseDate" defaultValue={todayDisplayDate()} />

                </div>

                <div className="space-y-2 sm:col-span-2">

                  <Label htmlFor="txnUnitPrice">Unit price per case (SGD)</Label>

                  <Input id="txnUnitPrice" name="txnUnitPrice" type="number" step="0.01" min="0" />
      ...
      {initial?.id ? (
        <div className="space-y-2">
          ...
          <Select id="status" name="status" defaultValue={initial?.status ?? "in_stock"}>
